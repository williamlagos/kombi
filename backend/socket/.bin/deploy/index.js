...
const remote = `https://git.heroku.com/${project.codename + "-" + (args.prod ? "prod-socket" : "dev-socket")}.git`;
...
const build = async () => {
    ...
    const commitMsg = "update(socket): Atualização do servidor de aplicação.";
    await exec("git init");
    await exec("git add .");
